...
var lcpsPreviewImg = sharedDirPath + "previewImg.php";
//************** tools **************//
...
function HttpHelper() {
	function httpPostRequest(option) {
	    $.ajax({
	        type: "POST",
	        timeout: option.timeout!=undefined ? option.timeout : 30000,
	        url: option.url,
	        dataType: "json",
	        data: option.data,
	...
	var LogLevelError = 10;
	var LogLevelInfo = 5;
	this.getLogLevelError = function () { return LogLevelError };
	this.getLogLevelInfo = function () { return LogLevelInfo };
	function httpRequestFilter(option, logFix, errField, msgField, successCode, logLevel) {
	    if (option) {
	        ...
	        logLevel = logLevel == undefined ? LogLevelInfo : logLevel;
	...
	this.loggedHttpPost = function (option, logFix, errField, msgField, successCode, logLevel) {
	    option = httpRequestFilter(option, logFix, errField, msgField, successCode, logLevel);
	    httpPostRequest(option);
	}
	this.loggedHttpGet = function (option, logFix, errField, msgField, successCode, logLevel) {
	    option = httpRequestFilter(option, logFix, errField, msgField, successCode, logLevel);
